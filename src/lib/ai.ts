...
6. TARGET DEPTH: Write rigorous, comprehensive paragraphs integrating verified empirical statistics. Focus on substance, not padding.`;
...
For EACH subsection, output the markdown subheading (e.g. \`### ${section.subsections[0]?.title || '1.1 Title'}\`) followed by rich, justified paragraphs (~${targetSubWords} words) addressing that subsection's brief directly. Integrate empirical data naturally. Include structured tables or mathematical equations where appropriate.`;
...
- Include structured Markdown Data Tables to present empirical distributions cleanly.`;
    ...
    formatInstruction = `- Write comprehensive, articulate paragraphs with structured points (Target: ~${targetWords} words). Prioritize deep analysis and empirical data over conversational padding.`;
  }
...
- Output ONLY the section body markdown.`;
  ...
  let prefix = `**${section.title}** examines key variables influencing operational outcomes.`;
  if (docType === "Academic Essay") {
    prefix = `**${section.title}** presents a critical analysis of empirical evidence.`;
  } else if (docType === "Literature Review") {
    prefix = `Scholarship regarding **${section.title}** highlights key dynamics and modern methodology.`;
  } else if (docType === "Freeform Summary") {
    prefix = `**Core Finding for ${section.title}**: The data indicates clear trends in structural performance.`;
  }
  ...
  if (section.subsections && section.subsections.length > 0) {
    ...
    return section.subsections.map((sub, sIdx) => {
      ...
      if (t.includes("executive") || t.includes("direct")) {
        ...
        const p3_variants = [
          `Rigorous evaluation relies on continuous measurement, automated testing, and predictable monitoring signals aligned with the project's core research goals.`,
          `Continuous validation of ${cleanSub.toLowerCase()} ensures that the architectural standards adapt smoothly as empirical requirements expand over time.`,
          `Through methodical observation, the structural integrity of these processes can be verified against the theoretical baselines established earlier in this review.`,
          `By documenting these interactions, future iterations can systematically eliminate bottlenecks and reinforce the overall robustness of the framework.`,
          `Ultimately, the integration of these models provides a coherent pathway for translating theoretical insights into measurable, reproducible outcomes.`
        ];
        p3 = p3_variants[variant];
      }
